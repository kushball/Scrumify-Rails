class TimeBlocksController < ApplicationController
    ...
    if params[:back_button]
      ...
      if !@schedule_type.blank?
        ...
        if !@blocks.empty?
          ...
          flash[:alert] = "You must have at least one small chunk of time free this week :)"
  ...
  def update
    ...
    if @time_block.update_attributes(params[:time_block])
      ...
      user = User.find(Schedule.find(Day.find(@time_block.day_id).schedule_id).user_id)
      group = user.groups.first
      group.update_group_schedule(user.schedule)
      
      redirect_to user, :notice  => "Successfully updated time block."
    else
      render :action => 'edit'
    end
  end
  ...
  def destroy
    ...
    redirect_to user, :notice => "Successfully destroyed time block."
  end
end
